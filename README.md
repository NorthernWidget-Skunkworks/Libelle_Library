...
Faults are there if you want them. `faulted(chip)` takes 0 for the VEML6075, 1 for the VEML6030, 2 for the ADS1115 and 3 for the ADXL343. `anyFault()`, `reportChip()` and `reportKind()` summarise the last reading. `printReport(Serial)` prints the report as text, and `reportNote()` gives it as one word, such as `VEML6075NoACK`, for a logger's note column. The bridge latches the faults of its three chips. On hardware v1 the library judges the accelerometer itself. `begin()` refuses a device that is not Schema 1, is not a Libelle, or runs firmware older than patch `LIBELLE_FW_MIN_PATCH`. `beginFailure()` tells you which: `NoACK`, `NotSchema1`, `WrongName`, `OldFirmware` or `NoAccel`. `printStatus(out)` prints one status line for a logger's status file (name, serial, versions, the last report as code and note, Pages 0 to 2 in hex), for the logger to write with its timestamp whenever `reportKind()` is not zero. This version needs the [NW_Core](https://github.com/NorthernWidget/NW_Core) library and Libelle firmware patch 1 or later. Firmware before Schema 1 will not pass `begin()`.
