...
The value getters return the stored reading: the mean over the readings of the last `updateMeasurements()`. Pass `true` to take a fresh one first. A getter with no reading returns `LIBELLE_ERROR` (-9999). That happens when the chip faulted, the device never answered, or the accelerometer was unresponsive.

### Readings, statistics, and faults

One `updateMeasurements()` can take several readings of each chip. `setUVReadings(n)` sets how many the VEML6075 takes. `setLightReadings(n)`, `setIRReadings(n)` and `setTiltReadings(n)` do the same for the VEML6030, the ADS1115 and the ADXL343. Each group stores up to `LIBELLE_UV_CAPACITY` readings, or its sibling constant: 8 by default, and you can define a larger one before the include. The value getters then return means. For the spread, `getUVAMean()`, `getUVAStd()`, `getUVASterr()` and `getUVAMedian()` read the stored readings. The same four exist for `UVB`, `ALS`, `White`, `Lux`, `IR_Short`, `IR_Mid`, `Temp`, `Roll` and `Pitch`, and `getUVCount()` and its siblings say how many readings each group holds. `setUVStats(true)` and its siblings add std and sterr columns to `getHeader()` and `getString()`. To read part of the board, pass `updateMeasurements()` a `Component` such as `Libelle::VEML6030`, or several ORed together. To log one row per reading to a file, call `beginReadings(component, n)`, `printHeader(out)`, then `logReading(out)` n times and `endReadings()`. Here `out` is any `Print`, such as an SdFat `File` or `Serial`.

Faults are there if you want them. `faulted(chip)` takes 0 for the VEML6075, 1 for the VEML6030, 2 for the ADS1115 and 3 for the ADXL343. `anyFault()`, `reportChip()` and `reportKind()` summarise the last reading. `printReport(Serial)` prints the report as text, and `reportNote()` gives it as one word, such as `VEML6075NotAnswering`, for a logger's note column. The bridge latches the faults of its three chips. On hardware v1 the library judges the accelerometer itself. `begin()` refuses a device that is not Schema 1, is not a Libelle, or runs firmware older than patch `LIBELLE_FW_MIN_PATCH`. `beginFailure()` tells you which: `NoACK`, `NotSchema1`, `WrongName`, `OldFirmware` or `NoAccel`. `printStatus(out)` prints one status line for a logger's status file (name, serial, hardware version, firmware patch and build commit from Page 0, this library's `LIBELLE_LIBRARY_VERSION` and `LIBELLE_LIBRARY_COMMIT`, the last report as code and note, Pages 0 to 2 in hex), for the logger to write with its timestamp whenever `reportKind()` is not zero. The two commits come from the NW-Build wrapper and are blank in an IDE build. Pages renumbered 2026-09-23 (spec 4c3b18d): calibration is Page 1 at 0x20, data Page 2 at 0x40. This version needs the [NW_Core](https://github.com/NorthernWidget/NW_Core) library and Libelle firmware patch 1 or later. Firmware before Schema 1 will not pass `begin()`.
